...
export async function getProjects(): Promise<Project[]> {
  ...
  return (data ?? []).map((r) => ({ ...r, skills: r.skills ?? [], roadmap: r.roadmap ?? [] }));
}

export async function upsertProject(project: Project) {
  const supabase = createServiceClient();
  return supabase.from("projects").upsert(project).select().single();
}
...
export async function getAchievements(): Promise<Achievement[]> {
  ...
  return (data ?? []).map((r) => ({ ...r, skills: r.skills ?? [] }));
}

export async function upsertAchievement(ach: Achievement) {
  const supabase = createServiceClient();
  return supabase.from("achievements").upsert(ach).select().single();
}
...
export async function getSkillCategories(): Promise<SkillCategory[]> {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .order("name");
  if (error) { console.error("getSkillCategories:", error.message); return []; }
  return (data ?? []).map((r) => ({ ...r, skills: r.skills ?? [] }));
}

export async function upsertSkillCategory(cat: SkillCategory) {
  const supabase = createServiceClient();
  return supabase.from("skill_categories").upsert(cat).select().single();
}

export async function deleteSkillCategory(id: string) {
  const supabase = createServiceClient();
  return supabase.from("skill_categories").delete().eq("id", id);
}
...
export async function upsertExperience(exp: Experience) {
  const supabase = createServiceClient();
  return supabase.from("experience").upsert(exp).select().single();
}
...
export async function upsertEducation(edu: Education) {
  const supabase = createServiceClient();
  return supabase.from("education").upsert(edu).select().single();
}
...
export async function getContact(): Promise<ContactInfo | null> {
  ...
  const { data, error } = await supabase
    ...
    .limit(1)
    .single();
  if (error) { console.error("getContact:", error.message); return null; }
  return data;
}

export async function upsertContact(contact: ContactInfo) {
  const supabase = createServiceClient();
  return supabase.from("contact_info").upsert(contact).select().single();
}
...
export async function uploadResume(file: File): Promise<string | null> {
  ...
  if (error) { console.error("uploadResume:", error.message); return null; }
